...
const SearchUser = (props) => {
  ...
  const searchUsers = () => {
    const res = props.allUsersQuery.users.filter((user) => user.username.toLowerCase().includes(searchText));
    setUserList(res);
  };
          ...
          && userList.map((user) => (
            <div className="user-card" key={user._id}>
              ...
              <div className="right">
                ...
                <div>
                  <span className="custom-button-follow btn-blue " onClick={() => followUser(user)}><p>{followingArray.includes(user._id) ? 'unfollow' : 'follow'}</p></span>
                  <br />
                </div>
              </div>
